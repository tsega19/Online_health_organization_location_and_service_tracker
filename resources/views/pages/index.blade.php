  ...
  <form action="{{ route('search') }}" method="GET">
    ...
    {{-- <form action="{{ route('web.find') }}" method="GET" >
          <div class="input-group input-navbar", class="col-md-4 col-sm-6 ...">
            ...
            <input type="text" name="query" class="form-control" placeholder="search by name.." aria-label="Username" aria-describedby="icon-addon1"> 
        ...
        {{-- <div class="pagination-block"> --}}
            ...
            {{  $orgs->appends(request()->input())->links('layouts.paginationlinks') }}
        </div>
  ...
  {{-- <div class="col-6 col-sm-3 py-1 wow fadeInRight" data-wow-delay="150ms">
          ...
          <div class="col-6 col-sm-3 py-1 wow fadeInRight" data-wow-delay="150ms">
            <select name="Services" id="Services" class="custom-select">
             ...
             <input type ="radio" value ="all" name ="all"> Using Location 
  ...
  <div class="page-hero bg-image overlay-dark" style="background-image: url(../assets/img/bg_image_1.jpg);">
    <div class="hero-section">
      <div class="container text-center wow zoomIn">
        <span class="subhead">Let's make your Simpler</span>
    ...
    <div class="item">
      <div class="card-doctor">
        <div class="header">
          ...
          <div class="meta">
            <a  href="/admin/{{$orgs->id}}">{{$orgs->email}}</a>
            <a href="#"><span class="mai-logo-whatsapp"></span></a>
          </div>
        </div>
        <div class="body">
          <p class="text-xl mb-0"><a href="/admin/{{$orgs->id}}">{{$orgs->name}}</h3></p>
          <span class="text-sm text-grey"> <a href="/admin/{{$orgs->id}}">{{$orgs->orgtype}}</span>
        </div>
      </div>
    </div>
                {{-- <a href="/admin/{{$orgs->id}}">{{$orgs->name}}</h3>
                   
                    <a href="/admin/{{$orgs->id}}">{{$orgs->phone}} --}}
     ...
     {{-- <div class="col-md-4 col-sm-4">
                     {{-- <img style="width 50%" src="/storage/images/{{$orgs->image}}" >  --}}
                  {{-- </div>  
               <small>written on {{$orgs->created_at}}</small>
                </div>
            </div> --}}
        {{-- </div> --}} -
    @endforeach 
